...
import com.example.core.data.local.MediaDao
import com.example.domain.model.AiClassification
import com.example.domain.model.AiFace
import com.example.domain.model.AiMetadata
import com.example.domain.model.AiObject
import com.example.domain.model.DuplicateGroupWithMedia
import com.example.domain.model.DuplicateMemberWithMedia
import com.example.domain.model.IndexingProgress
import com.example.domain.model.IndexingStatus
import com.example.domain.model.MediaItem
import com.example.domain.model.MediaItemWithAi
    ...
    override suspend fun scanAndIndexGallery(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            _indexingProgress.value = IndexingProgress(
                ...
                message = "Scanning device storage for media..."
            )

            val scannedItems = galleryScanner.scanMediaStore()
            if (scannedItems.isEmpty()) {
                _indexingProgress.value = IndexingProgress(
                    status = IndexingStatus.COMPLETED,
                    message = "Scan complete. No local media found."
                )
                return@withContext Result.success(0)
            }

            mediaDao.insertAll(scannedItems)
            val total = scannedItems.size

            _indexingProgress.value = IndexingProgress(
                status = IndexingStatus.GENERATING_THUMBNAILS,
                scannedCount = total,
                totalCount = total,
            ...
            scannedItems.forEachIndexed { index, item ->
                _indexingProgress.value = IndexingProgress(
                    ...
                    message = "Indexing EXIF metadata (${index + 1}/$total)"
                )

                val thumbPath = thumbnailEngine.getOrCreateThumbnail(item)
                var updatedItem = if (thumbPath != null) item.copy(thumbnailPath = thumbPath) else item
                updatedItem = metadataIndexer.extractExif(updatedItem)
                mediaDao.insert(updatedItem)
            }
            ...
            _indexingProgress.value = IndexingProgress(
                ...
                scannedCount = total,
                totalCount = total,
                message = "Gallery index up to date ($total items)"
            )
            ...
            _indexingProgress.value = IndexingProgress(
                ...
                message = "Scanning failed: ${e.localizedMessage}"
            )
            Result.failure(e)
        }
    }
