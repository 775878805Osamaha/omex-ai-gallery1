package com.example.ui.feature_indexing
...
fun IndexingStatusScreen(
    ...
    val thumbnailEngine = remember { ThumbnailEngine(context) }
    val cacheSizeBytes = remember(indexingProgress) { thumbnailEngine.getThumbnailCacheSize() }

    val indexedCount = mediaItems.count { it.isIndexed }
    val totalCount = mediaItems.size

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Indexer & System Status", color = TextPrimaryDark, fontWeight = FontWeight.Bold) },
                navigationIcon = {
        ...
        Column(
            ...
            Card(
                ...
                Row(
                    ...
                    Column {
                        Text("OMEX Offline Engine", color = TextPrimaryDark, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                        Text("Phase 1: Scanning & Metadata Indexing", color = TextMutedDark, style = MaterialTheme.typography.bodySmall)
                    }
                }
            }
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(20.dp)) {
                    Text("Index Status", color = TextPrimaryDark, style = MaterialTheme.typography.titleSmall, fontWeight = FontWeight.Bold)
                    Spacer(modifier = Modifier.height(8.dp))
                    Text(
                        text = indexingProgress.message.ifEmpty { "Index status active and healthy." },
                        color = CyanAccent,
                        fontSize = 14.sp
                    )
                }
            ...
            // Rescan Button
            Button(
                onClick = { galleryViewModel.triggerGalleryScan() },
                ...
                Text("Trigger Full Re-Index", color = ObsidianBg, fontWeight = FontWeight.Bold)
            }
        }
    }
}
